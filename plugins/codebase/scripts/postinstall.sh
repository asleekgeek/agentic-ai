...
#
# Builds automatised-pipeline from the vendored Rust source so that on first MCP
# connection Claude Code finds the compiled binary at
# ${PLUGIN_ROOT}/src-rust/target/release/automatised-pipeline and launches it
...
if [ -x "${prebuilt_in_src}" ]; then
  echo "codebase plugin: automatised-pipeline already built — skipping." >&2
  exit 0
fi

if ! command -v cargo >/dev/null 2>&1; then
  echo "codebase plugin: cannot pre-build automatised-pipeline — cargo not on PATH." >&2
...
echo "codebase plugin: building automatised-pipeline from source (one-time, ~2-5 min)..." >&2
