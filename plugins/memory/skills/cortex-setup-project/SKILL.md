...
1. Detection: attempt `memory:ingest_codebase({"project_path": "<cwd>"})`. If it succeeds, record the counts (wiki pages, memory entities, KG edges) for the summary and skip to Phase 4.
...
1. Call `memory:backfill_memories({"dry_run": true, "max_files": 500})` to preview available session files.
2. If files are available, call `memory:backfill_memories({"max_files": 500, "min_importance": 0.35})` to import.
...
2. Call `memory:memory_stats({})` to get the final system state.
