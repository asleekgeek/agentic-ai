 ...
 *
 * Port of: cortex main mcp_server/core/synaptic_plasticity_hebbian.py
 *
 ...
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:168-185
 *
...
export const MAX_WEIGHT = 5.0; // source: cortex main mcp_server/core/synaptic_plasticity.py:_MAX_WEIGHT
export const MIN_WEIGHT = 0.01; // source: cortex main mcp_server/core/synaptic_plasticity.py:_MIN_WEIGHT

// ── Rate constants ─────────────────────────────────────────────────────────

// Hand-tuned engineering constants — no paper derivation cited in source.
const LTP_RATE: number = 0.05; // source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:37
const LTD_RATE: number = 0.02; // source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:38
const BCM_THETA_DECAY: number = 0.95; // source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:39
...
const STDP_A_PLUS: number = 0.03; // source: Bi & Poo (1998) A+ amplitude; cortex main synaptic_plasticity_hebbian.py:40
const STDP_A_MINUS: number = 0.02; // source: Bi & Poo (1998) A- amplitude; cortex main synaptic_plasticity_hebbian.py:41
const STDP_TAU_PLUS: number = 24.0; // source: cortex main synaptic_plasticity_hebbian.py:42 (adapted ms → 24h)
const STDP_TAU_MINUS: number = 24.0; // source: cortex main synaptic_plasticity_hebbian.py:43 (adapted ms → 24h)

// ── Derived constants ──────────────────────────────────────────────────────

// Default BCM theta_m: midpoint of [0,1] — used as the default sliding threshold.
// source: cortex main synaptic_plasticity_hebbian.py:63,84 (theta=0.5 default)
const DEFAULT_THETA: number = 0.5;

// Default pre- and post-synaptic activity level (neutral / unknown).
// source: cortex main synaptic_plasticity_hebbian.py:60-62 (pre_activity=1.0 default)
const DEFAULT_ACTIVITY: number = 1.0;

// Rounding factor for 6 decimal places (round(x, 6) → x * 10^6 / 10^6).
// source: cortex main synaptic_plasticity_hebbian.py:150-151 (round(new_w, 6))
const ROUND_6DP: number = 1_000_000; // source: cortex main synaptic_plasticity_hebbian.py:150 (round factor for 6dp)

// Hours-per-day denominator for the inactivity decay time scale.
// source: cortex main synaptic_plasticity_hebbian.py:105 (/ 24.0 normalisation)
const HOURS_PER_DAY: number = 24.0; // source: cortex main synaptic_plasticity_hebbian.py:105

// STDP temporal resolution threshold: dt below this is treated as simultaneous.
// source: cortex main synaptic_plasticity_hebbian.py:212,232-236 (abs(dt) < 0.001)
const STDP_DT_THRESHOLD: number = 0.001; // source: cortex main synaptic_plasticity_hebbian.py:212
 ...
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:58-76
 ...
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:79-106
 * postcondition: returned weight is in [minWeight, currentWeight].
 */
// Zero post-activity default: no signal means inactivity path.
// source: cortex main synaptic_plasticity_hebbian.py:84 (post_activity=0.0 default)
const DEFAULT_POST_ACTIVITY_LTD: number = 0.0; // source: cortex main synaptic_plasticity_hebbian.py:84
...
export function computeLtd(
  ...
  const decay = ltdRate * Math.log1p(timeSinceCoAccessHours / HOURS_PER_DAY); // source: cortex main synaptic_plasticity_hebbian.py:105
 ...
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:109-119
 */
 ...
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:122-155
 */
function hebbianSingle(
  ...
  // source: cortex main synaptic_plasticity_hebbian.py:134
  ...
  const roundedWeight = Math.round(newWeight * ROUND_6DP) / ROUND_6DP; // source: cortex main synaptic_plasticity_hebbian.py:150 (round(new_w,6))
  const roundedDelta = Math.round((newWeight - w) * ROUND_6DP) / ROUND_6DP; // source: cortex main synaptic_plasticity_hebbian.py:151 (round(new_w-w,6))
 ...
 *   action ("ltp" | "ltd" | "none").
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:158-196
 */
export function applyHebbianUpdate(
  ...
  // source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:170-185
 ...
 *         cortex main mcp_server/core/synaptic_plasticity_hebbian.py:200-218
...
export function computeStdpUpdate(
  ...
  if (Math.abs(deltaT) < STDP_DT_THRESHOLD) return currentWeight; // source: cortex main synaptic_plasticity_hebbian.py:212
  const deltaW =
    deltaT > 0
      ? aPlus * Math.exp(-deltaT / tauPlus)
      : -aMinus * Math.exp(deltaT / tauMinus);
  const newWeight = currentWeight + deltaW;
  return Math.max(minWeight, Math.min(maxWeight, Math.round(newWeight * ROUND_6DP) / ROUND_6DP)); // source: cortex main synaptic_plasticity_hebbian.py:218
}
 ...
 *
 * source: cortex main mcp_server/core/synaptic_plasticity_hebbian.py:247-256
 */
export function applyStdpBatch(
  ...
  return pairs.map((p) => {
    ...
    // source: cortex main synaptic_plasticity_hebbian.py:232-236
    const direction =
    ...
    return {
      ...
      delta: Math.round((newWeight - w) * ROUND_6DP) / ROUND_6DP, // source: cortex main synaptic_plasticity_hebbian.py:242
      direction,
    };
  });
}
